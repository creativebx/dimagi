...
class Partners_Widget extends WP_Widget {
	function Partners_Widget() {
		$widget_ops = array('classname' => 'partners_widget', 'description' => __('Show your partners on your site.','anya'));
		parent::WP_Widget(false, 'DESIGNARE _ Partners', $widget_ops);
	}
...
?>  
       ...
       <p>
	        <label>&#8212; <?php _e('Partners Effect','anya'); ?> &#8212;<br>
       ...
       <p><label for="<?php echo $this->get_field_id('nshow'); ?>">&#8212; <?php _e('Number Partners to show','anya'); ?> &#8212;<input class="widefat" id="<?php echo $this->get_field_id('nshow'); ?>" name="<?php echo $this->get_field_name('nshow'); ?>" type="text" value="<?php echo $nshow; ?>" /><br><span class="flickr-stuff">If 0 will show all partners.</span></label></p>
       <p class="partners_scroller_select"><label for="<?php echo $this->get_field_id('scroller'); ?>">&#8212; <?php _e('Scroller','anya'); ?> &nbsp;<input id="<?php echo $this->get_field_id('scroller'); ?>" name="<?php echo $this->get_field_name('scroller'); ?>" type="checkbox" value="scroller" <?php if($scroller == "scroller") echo 'checked'; ?> onchange="if (!jQuery(this).is(':checked')) jQuery(this).closest('p').next().show(); else jQuery(this).closest('p').next().hide();"/></label></p>
       
       <!-- NEW -->
       <p><label>&#8212; <?php _e('Partners per Row','anya'); ?> &#8212;</label><br>
	...
	function widget($args, $instance) {
		...
		if (esc_html($title) != "" && esc_html($title) != " "){ $titles_html .= '<div class="anyatitle"><hr><span>'.esc_html($title).'</span>'.$pag.'</div>'; }
		else { $titles_html .= '<div class="anyatitle">'.$pag.'</div>'; }
	
		$output = "<section id='partners-".$rid."' class='shortcode-partners sixteen columns'>" . $titles_html . "<div class='partners-carousel deswidget'><ul class='partners-items'>";
