...
class Video_Widget extends WP_Widget {
	function Video_Widget() {
		$widget_ops = array('classname' => 'video_widget', 'description' => __('Show your videos from Youtube or Vimeo.', 'anya'));
		parent::WP_Widget(false, 'DESIGNARE _ Video', $widget_ops);
	}
...
?>  
       ...
       <p><label>&#8212; <?php _e('Video Hoster', 'anya'); ?> &#8212;<br>
		    		<input type="radio" name="<?php echo $this->get_field_name('v_type'); ?>" value="you" <?php if($order == 'you' || !$order) echo 'checked'; ?>> <?php _e('Youtube', 'anya'); ?><br>
		    		<input type="radio" name="<?php echo $this->get_field_name('v_type'); ?>" value="vim" <?php if($order == 'vim') echo 'checked'; ?>> <?php _e('Vimeo', 'anya'); ?><br>
		    </label></p>
		    <p><label for="<?php echo $this->get_field_id('v_id'); ?>">&#8212; <?php _e('Video ID', 'anya'); ?> &#8212;<input class="widefat" id="<?php echo $this->get_field_id('v_id'); ?>" name="<?php echo $this->get_field_name('v_id'); ?>" type="text" value="<?php echo $v_id; ?>" /></label></p>
