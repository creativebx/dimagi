...
class RecentComments_Widget extends WP_Widget {
	function RecentComments_Widget() {
		$widget_ops = array('classname' => 'recentComments_widget', 'description' => __('Show your recent blog comments on your site.','anya'));
		parent::WP_Widget(false, 'DESIGNARE _ Recent Comments', $widget_ops);
	}
...
?>  
        ...
        <p><label for="<?php echo $this->get_field_id('nposts'); ?>">&#8212; <?php _e('Number Posts to show','anya'); ?> &#8212;<input class="widefat" id="<?php echo $this->get_field_id('nposts'); ?>" name="<?php echo $this->get_field_name('nposts'); ?>" type="text" value="<?php echo $nposts; ?>" /><br><span class="flickr-stuff">If 0 will show all posts.</span></label></p>
        		    <p><label>&#8212; <?php _e('Order by','anya'); ?> &#8212;<br>
			...
			<?php 
				...
				foreach($comments as $comment) :
					...
					//if (!function_exists('icl_object_id'))					
						echo('<li ><a class="the_title" href="'.home_url() . '/?p='.$comment->comment_post_ID.'">'.$source.'	</a><a class="the_title" href="'.home_url() . '/?p='.$comment->comment_post_ID.'">'.get_the_title($comment->comment_post_ID) . '</a><br><span class="blog-i">'. __(get_option(DESIGNARE_SHORTNAME."_by_text"), "anya"). ':&nbsp;</span><a class="the_author" href="' . $current_link.'">'.$comment->comment_author.'</a></li>');
					endforeach;
