...
class Team_Widget extends WP_Widget {
	function Team_Widget() {
		$widget_ops = array('classname' => 'team_widget', 'description' => __('Show your team on your site.','anya'));
		parent::WP_Widget(false, 'DESIGNARE _ Team', $widget_ops);
	}
...
?>  
       ...
       <p><label for="<?php echo $this->get_field_id('nshow'); ?>">&#8212; <?php _e('Number Team to show','anya'); ?> &#8212;<input class="widefat" id="<?php echo $this->get_field_id('nshow'); ?>" name="<?php echo $this->get_field_name('nshow'); ?>" type="text" value="<?php echo $nshow; ?>" /><br><span class="flickr-stuff">If 0 will show all your team members.</span></label></p>
       <p class="team_scroller_select"><label for="<?php echo $this->get_field_id('scroller'); ?>">&#8212; <?php _e('Scroller','anya'); ?> &nbsp;<input id="<?php echo $this->get_field_id('scroller'); ?>" name="<?php echo $this->get_field_name('scroller'); ?>" type="checkbox" value="scroller" <?php if($scroller == "scroller") echo 'checked'; ?> onchange="if (!jQuery(this).is(':checked')) jQuery(this).closest('p').next().show(); else jQuery(this).closest('p').next().hide();"/></label></p>
       
       <!-- NEW -->
       <p><label>&#8212; <?php _e('Team per Row','anya'); ?> &#8212;</label><br>
	...
	function widget($args, $instance) {
		...
		if ($scroller == "yes"){
			...
			$output .= '<div class="team_header anyatitle"><span>' . esc_html( $title ) . '</span><div class="pag-proj_team">
			...
			$output .= '<h2 class="team_header anyatitle sixteen columns" style="min-height:20px;"><span>' . esc_html( $title ) . '</span></h2>';
		}
