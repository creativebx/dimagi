...
class Twitter_Widget extends WP_Widget {
	function Twitter_Widget() {
		$widget_ops = array('classname' => 'twitter_widget', 'description' => __('Show your tweets on your site.', 'anya'));
		parent::WP_Widget(false, 'DESIGNARE _ Twitter', $widget_ops);
	}
...
?>  
       ...
       <p><label for="<?php echo $this->get_field_id('ntweets'); ?>">&#8212; <?php _e('Number Tweets to show','anya'); ?> &#8212;<input class="widefat" id="<?php echo $this->get_field_id('ntweets'); ?>" name="<?php echo $this->get_field_name('ntweets'); ?>" type="text" value="<?php echo $ntweets; ?>" /><br><span class="flickr-stuff">If 0 will show the last tweet.</span></label></p>
       <p><label for="<?php echo $this->get_field_id('linkprofile'); ?>">&#8212; <?php _e('Link to Profile','anya'); ?> &nbsp;<input id="<?php echo $this->get_field_id('linkprofile'); ?>" name="<?php echo $this->get_field_name('linkprofile'); ?>" type="checkbox" value="yes" <?php if($linkprofile == "yes") echo 'checked'; ?>/><br><span class="flickr-stuff">Title with link to your profile.</span></label></p> 
...
function widget($args, $instance) {
    ...
    echo $before_widget;
    if ($twitter_data_is_filled) wp_enqueue_script( 'destweet', get_template_directory_uri() .'js/twitter/jquery.tweet.js', array(),'1.0',$in_footer = true);
    ?>
			...
			<div id="twitter_update_list">
			 <?php
				 if ($twitter_data_is_filled){
					 ...
					 ?>
					 <p>Please fill the fields on the <strong>Appearance > Anya Options > Twitter and Social Icons > Twitter</strong> panel with your credentials.</p>
					 <?php
				 }
			 ?>
			</div>
