...
if ( isset($_GET['page']) && $_GET['page'] == DESIGNARE_OPTIONS_PAGE ){
	...
	add_action('init', 'designare_set_options'); 
}
...
function des_get_all_google_fonts(){
	...
	try{	
		...
		for ($i = 0; $i < count($fontArray); $i++){
			 array_push($anya_google_fonts, array('id'=>$fontArray[$i]['css-name'],'name' => $fontArray[$i]['font-name']));
		}	
	} catch(Exception $e) {
		$anya_google_fonts = array(	array( 'name' => "Cantarell", 'variant' => ':r,b,i,bi'),
