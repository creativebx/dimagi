...
?>
	...
	<div id="big_footer">	
		<?php
		wp_reset_query();
		if (isset($post)) $customnews = get_post_meta($post->ID, 'des_custom_newsletter_value', true);
					...
					<?php
						if ($twitter == "on" && $nl == "on"){
							...
							<div class="eight columns">
								...
								<div class="bird-img"><?php _e(get_option(DESIGNARE_SHORTNAME."_latest_for_twitter"), "anya"); ?></div>
								<?php if (!$twitter_data_is_filled){
									echo "<p>Please fill the fields on the <strong>Appearance > Anya Options > Twitter and Social Icons > Twitter</strong> panel with your credentials.</p>";
								} ?> 
    ...
    <?php
	    if (des_get_value(DESIGNARE_SHORTNAME."_show_sec_footer") == "on"){
		    ...
		    <div class="copys">
				<div class="container">
					<?php if (get_option(DESIGNARE_SHORTNAME."_copyrights_left")){ ?><div class="copys_left eight columns"><?php _e(get_option(DESIGNARE_SHORTNAME."_copyrights_left"), "anya"); ?></div><?php } ?>
...
if (is_archive() || is_single() || is_search() || is_page_template('blog-template.php') || is_page_template('blog-template-fullwidth.php') || is_page_template('blog-template-leftsidebar.php') ) {
	...
	if (isset($_GET['paged'])) $anya_pag = $_GET['paged'];
	else {
		$pageURL = 'http';
		...
		$anya_pagina = explode('/page/', $pageURL);
		if(isset($anya_pagina[1])) $anya_pagina = explode ('/', $anya_pagina[1]);
		if ($anya_pagina[0]) $anya_pag = $anya_pagina[0];	
	}
	if (!is_numeric($anya_pag)) $anya_pag = 1;
	$max = 0;
	
	switch ($anya_reading_option){
		case "scrollauto": 
				...
				if( !is_singular() ) {	
					...
					if (is_search()){
						...
						$anya_reading_option = get_option('anya_blog_reading_type');
						...
						$anya_paged = ( get_query_var('paged') > 1 ) ? get_query_var('paged') : 1;
	...
	?>
	<div class="anya_helper_div" id="loader-startPage"><?php echo $anya_pag; ?></div>
	<div class="anya_helper_div" id="loader-maxPages"><?php echo $max; ?></div>
	<div class="anya_helper_div" id="loader-nextLink"><?php echo next_posts($max, false); ?></div>
	<?php
}
...
<?php 
	...
	if ($anya_pag > 1) {
	?>
	<div class="anya_helper_div" id="loader-prevLink"><?php
		if (isset($_GET['paged'])) {
			$pageURL = 'http';
			if (isset($_SERVER["HTTPS"]) && $_SERVER["HTTPS"] == "on") {$pageURL .= "s";}
			$pageURL .= "://";
...
<div class="anya_helper_div" id="reading_option"><?php 
	if ($anya_reading_option == "scrollauto"){
		$detect = new Mobile_Detect();
		if ($detect->isMobile())
			$anya_reading_option = "scroll";
	}
	echo $anya_reading_option; 
?></div>
<div class="anya_helper_div" id="anya_no_more_posts_text"><?php echo __(get_option('anya_no_more_posts_text'), "anya"); ?></div>
<div class="anya_helper_div" id="anya_load_more_posts_text"><?php echo __(get_option('anya_load_more_posts_text'), "anya");  ?></div>
<div class="anya_helper_div" id="anya_loading_posts_text"><?php echo __(get_option('anya_loading_posts_text'), "anya");  ?></div>
<div class="anya_helper_div" id="anya_links_color_hover"><?php echo des_get_value('anya_links_color_hover'); ?></div>
<div class="anya_helper_div" id="anya_enable_images_magnifier"><?php echo get_option('anya_enable_images_magnifier'); ?></div>
<div class="anya_helper_div" id="anya_thumbnails_hover_option"><?php echo get_option('anya_thumbnails_hover_option'); ?></div>
<div class="anya_helper_div" id="anya_menu_color">#<?php echo des_get_value(DESIGNARE_SHORTNAME."_menu_color"); ?></div>
<div class="anya_helper_div" id="anya_fixed_menu"><?php echo get_option(DESIGNARE_SHORTNAME."_fixed_menu"); ?></div>
<div class="anya_helper_div" id="anya_thumbnails_effect"><?php if (get_option(DESIGNARE_SHORTNAME."_animate_thumbnails") == "on") echo get_option(DESIGNARE_SHORTNAME."_thumbnails_effect"); else echo "none"; ?></div>
<div class="anya_helper_div loadinger">
	<img alt="loading" src="<?php echo get_template_directory_uri(). '/img/ajx_loading.gif' ?>">
</div>
<div class="anya_helper_div" id="permalink_structure"><?php echo get_option('permalink_structure'); ?></div>
<div class="anya_helper_div" id="headerstyle3_menucolor">#<?php echo des_get_value(DESIGNARE_SHORTNAME."_menu_color"); ?></div>
<div class="anya_helper_div" id="disable_responsive_layout"><?php echo get_option('anya_disable_responsive'); ?></div>
