...
add_action( 'after_setup_theme', 'designare_anya_setup' );

function designare_anya_setup(){
	 ...
	 */
	load_theme_textdomain( 'anya', get_template_directory() . '/languages' );
	...
	define("DESIGNARE_THEMENAME", 'Anya');
	define("DESIGNARE_SHORTNAME", 'anya');
	$my_theme = wp_get_theme();
	define("DESIGNARE_VERSION", $my_theme->Version);
	
	global $des_theme_prefix; $des_theme_prefix = "anya_";
	...
	define("DESIGNARE_PATTERNS_URL", get_template_directory_uri().'/images/anya_patterns/');
	...
	define("DESIGNARE_GOOGLE_FONTS", "http://fonts.googleapis.com/css?family=Open+Sans:400,300,600,700,800".DESIGNARE_SEPARATOR."".DESIGNARE_SEPARATOR."http://fonts.googleapis.com/css?family=Raleway:400,300,700,800".DESIGNARE_SEPARATOR);
	...
	add_action('wp_enqueue_scripts', 'designare_anya_style', 1);
	add_action('wp_enqueue_scripts','designare_anya_custom_head', 2);
	...
	if (is_admin()){
		...
		add_action('admin_enqueue_scripts','designare_anya_admin_style');
...
function woocommerce_header_add_to_cart_fragment( $fragments ) {
	...
	<div class="anya_dynamic_shopping_bag">
        <div class="anya_little_shopping_bag_wrapper">
            <div class="anya_little_shopping_bag" style="background: transparent !important;">
                <div class="title">
            ...
            <div class="anya_minicart_wrapper">
                ...
                echo '<ul class="cart_list">';
                    ...
                    <a href="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>" class="button anya_minicart_cart_but"><?php _e('View Shopping Bag', 'anya'); ?></a>
                    <a href="<?php echo esc_url( $woocommerce->cart->get_checkout_url() ); ?>" class="button anya_minicart_checkout_but"><?php _e('Proceed to Checkout', 'anya'); ?></a>
                    <?php                                    
        ...
        <script type="text/javascript">
	        $(".anya_little_shopping_bag_wrapper").on("mouseenter mouseover", function() {
				if(!$(this).data('init')){
		            $(this).data('init', true);
		            $(this).hover(
		                function(){
							$('.anya_minicart_wrapper').fadeIn(200);
		                },
		                function(){
		                    $('.anya_minicart_wrapper').fadeOut(200);
	...
	<?php
	$fragments['div.anya_dynamic_shopping_bag' ] = ob_get_clean();
	return $fragments;
}

function designare_anya_admin_style(){
...
function designare_css_options(){
	global $anya_custom, $anya_styleColor, $post, $des_templater;
	$des_templater = array();
	...
	foreach($styleTabs as $tab){
		...
		if (get_post_meta(get_the_ID(), "enable_".$tab."_template_value", true) === "yes" && count($options) > 0){
			...
			switch ($tab){
				...
				break;
				case "menu":
					$panel_options_list = array($des."_menu_font", $des."_menu_font_size", $des."_menu_color", $des."_menu_selected_color", $des."_menu_background_color", $des."_menu_selected_background_color", $des."_menu_uppercase", $des."_menu_side_margin", $des."_big_menu_margin_top", $des."_big_menu_padding_bottom", $des."_small_menu_margin_top", $des."_small_menu_padding_bottom");
				break;
				case "pagetitle":
	...
	$anya_styleColor = "#".des_get_value(DESIGNARE_SHORTNAME."_style_defcolor");
	if ("#".des_get_value(DESIGNARE_SHORTNAME."_style_color") != $anya_styleColor) $anya_styleColor = "#".des_get_value(DESIGNARE_SHORTNAME."_style_color");
	...
	$headerStyleType = "";
	if (isset($post)){
		if (get_post_meta($post->ID, 'des_custom_header_style_value', true) != '' && get_post_meta($post->ID, 'des_custom_header_style_value', true) == "on"){
 		...
 		body{
			...
			if ($bodyLayoutType == "boxed"){?>
				...
				if ($bodytype == "color") echo "background: #" . des_get_value(DESIGNARE_SHORTNAME."_body_color") . ";";
	 			if ($bodytype == "pattern")
					echo "background: url('" . get_template_directory_uri() . "/images/anya_patterns/" . des_get_value(DESIGNARE_SHORTNAME."_body_pattern") . "') 0 0 repeat fixed;";
...
*/
				...
				if ($bodytype == "image"){
					if ((isset($_SERVER['HTTP_USER_AGENT']) && (strpos($_SERVER['HTTP_USER_AGENT'], 'Trident') !== false || strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE') !== false))){
						echo "background-size:cover !important; background-attachment: fixed !important; background-repeat: no-repeat !important;";
					} else {
						echo "background-repeat:no-repeat; background-position:center center; -o-background-size: 100% 100%, auto; -moz-background-size: 100% 100%, auto; -webkit-background-size: 100% 100%, auto; background-size: 100% 100%, auto;background-attachment: fixed !important;";	
					}
					echo "background: url(" . des_get_value(DESIGNARE_SHORTNAME."_body_image") . ") ;";  
				}
			} 
			if ($bodyLayoutType == "full") { 
				...
				if ($contentbgtype == "pattern") echo "background: url('" . get_template_directory_uri() . "/images/anya_patterns/" . des_get_value(DESIGNARE_SHORTNAME."_contentbg_pattern") . "') 0 0 repeat fixed !important;";
				...
				if ($contentbgtype == "image"){
					if ((isset($_SERVER['HTTP_USER_AGENT']) && (strpos($_SERVER['HTTP_USER_AGENT'], 'Trident') !== false || strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE') !== false))){
						echo "background-size:cover !important; background-attachment !important: fixed;background-repeat: no-repeat !important;";
					} else {
						echo "background-repeat:no-repeat; background-position:center center; -o-background-size: 100% 100%, auto; -moz-background-size: 100% 100%, auto; -webkit-background-size: 100% 100%, auto; background-size: 100% 100%, auto !important;background-attachment: fixed !important;";	
					}
			...
			if ($bodyLayoutType == "boxed"){?>
				...
				if ($bodytype == "image"){
					if ((isset($_SERVER['HTTP_USER_AGENT']) && (strpos($_SERVER['HTTP_USER_AGENT'], 'Trident') !== false || strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE') !== false))){
						...
						echo "background-repeat:no-repeat; background-position:center center; -o-background-size: 100% 100%, auto; -moz-background-size: 100% 100%, auto; -webkit-background-size: 100% 100%, auto; background-size: 100% 100%, auto; background-attachment: fixed;";	
					}
					echo "background: url(" . des_get_value(DESIGNARE_SHORTNAME."_body_image") . ") ;";  
				}
			} 
			if ($bodyLayoutType == "full") { 
				...
				if ($contentbgtype == "image"){
					if ((isset($_SERVER['HTTP_USER_AGENT']) && (strpos($_SERVER['HTTP_USER_AGENT'], 'Trident') !== false || strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE') !== false))){
						...
						echo "background-repeat:no-repeat; background-position:center center; -o-background-size: 100% 100%, auto; -moz-background-size: 100% 100%, auto; -webkit-background-size: 100% 100%, auto; background-size: 100% 100%, auto !important;background-attachment: fixed;";	
					}
					echo "background: url(" . des_get_value(DESIGNARE_SHORTNAME."_contentbg_image") . ");";
				}
			} 
		...
		<?php
			if (des_get_value(DESIGNARE_SHORTNAME."_show_sec_footer") == "off"){
				...
				if ($secfooterbgtype == "pattern") echo "background: url('" . get_template_directory_uri() . "/images/anya_patterns/" . des_get_value(DESIGNARE_SHORTNAME."_sec_footerbg_pattern") . "') 0 0 repeat;";
		...
		<?php
			...
			if ($toppanelbgtype == "pattern") echo "background: url('" . get_template_directory_uri() . "/images/anya_patterns/" . des_get_value(DESIGNARE_SHORTNAME."_toppanelbg_pattern") . "') 0 0 repeat fixed;";
		...
		<?php
			...
			if ($headerbgtype == "pattern") echo "background: url('" . get_template_directory_uri() . "/images/anya_patterns/" . des_get_value(DESIGNARE_SHORTNAME."_headerbg_pattern") . "') 0 0 repeat !important;";
		...
		<?php
			...
			if ($bigfootertype == "pattern") echo "background: url('" . get_template_directory_uri() . "/images/anya_patterns/" . des_get_value(DESIGNARE_SHORTNAME."_footerbg_pattern") . "') 0 0 repeat fixed !important;";
		...
		<?php
			...
			if ($toppanelbgtype == "pattern") echo "background: url('" . get_template_directory_uri() . "/images/anya_patterns/" . des_get_value(DESIGNARE_SHORTNAME."_toppanelbg_pattern") . "') 0 0 repeat fixed !important;";
		...
		#menu_top_bar .sub-menu li a:hover{color:<?php echo $anya_styleColor; ?> !important;}

		.more-chitem{background:<?php echo $anya_styleColor; ?> !important;}
		...
		.project_list_s2_style2 .numberofcomments{background: <?php echo $anya_styleColor; ?>;}
		.project_list_s2_style2 .numberofcomments_triangle{border-color: <?php echo $anya_styleColor; ?> transparent transparent transparent;}
		.posttype_date_triangle{border-color: transparent transparent <?php echo $anya_styleColor; ?> transparent;}
		...
		.mail_chimp_form_container{
			<?php
				...
				if ($twitternewslettertype == "pattern") echo "background: url('" . get_template_directory_uri() . "/images/anya_patterns/" . des_get_value(DESIGNARE_SHORTNAME."_twitter_newsletter_pattern") . "') 0 0 repeat !important;";
				...
				if ($twitternewslettertype == "image"){
					echo "background-repeat:no-repeat; background-position:center center; -o-background-size: 100% 100%, auto; -moz-background-size: 100% 100%, auto; -webkit-background-size: 100% 100%, auto; background-size: 100% 100%, auto !important;";
					echo "background: url(" . des_get_value(DESIGNARE_SHORTNAME."_twitter_newsletter_image") . ") no-repeat; background-size: 100% 100% !important;";  
				}
			?>
		}
		...
		h2.wpb_heading, .home_widget .page_info_title_s3, .home_widget .page_info_title_s4, page_info_title_testimonials, .anyatitle span, .vc_text_separator div, #accordion .acc-title h2, .acc-substitute .acc-title h2, .widget h2, .custom-widget h4, .widget h2, .widget h4, #secondary #recentPostsSidebar_widget h2, .custom-widget h4, .widget-flexslider h4, #recentPostsSidebar_widget h2{
			...
			echo "font-size:" .preg_replace( '/\s+/', '', des_get_value(DESIGNARE_SHORTNAME."_st_size")) .";"; 
		...
		.trigger_toppanel_closer .clicker{
			border-color: transparent <?php echo $anya_styleColor; ?> transparent transparent; 
		}
		...
		.woocommerce table.cart a.remove:hover, .woocommerce #content table.cart a.remove:hover, .woocommerce-page table.cart a.remove:hover, .woocommerce-page #content table.cart a.remove:hover, .camera_prev > span:hover, .camera_next > span:hover, #wrapper .cameracontrols .camera-controls-toggler, .recentPosts_style2 .posttype_date{
			<?php echo "background-color: " . $anya_styleColor . " !important;"; ?>
		}
		...
		.camera_prev:hover > span, .camera_next:hover > span, #send-comment, .wpcf7-submit, #back-to-top a:hover, .shop_topbar_middle, .shop_topbar_rightcorner, .shop_bottombar_leftcorner, .shop_bottombar_middle, .trigger_toppanel_closer .clicker .signal, ul.products li.hentry:hover .added_to_cart, ul.products li.hentry .added_to_cart, .tp-bullets.simplebullets.round .bullet:hover, .tp-bullets.simplebullets.round .bullet.selected, .tp-bullets.simplebullets.navbar .bullet:hover, .tp-bullets.simplebullets.navbar .bullet.selected, .pop-menu, .filterby_btn{
 		...
 		.cameraholder, .services-graph li span, .socialdiv a[title]:hover:after, span.shortcode-highlight, .dl-menuwrapper li.dl-back > a, .headerstyle-style3 header #menulava > li.current-menu-item > a, .headerstyle-style3 header #menulava > li.current-menu-ancestor > a{
	 		<?php echo "background: " . $anya_styleColor . " !important;"; ?>
 		}
 		
 		.settings-open{
	 		<?php echo "background: " . $anya_styleColor . " url(".get_template_directory_uri()."/img/close.png) no-repeat center center;"; ?>
 		}

 		<?php if ($headerStyleType === "style1" || $headerStyleType === "style2"){
	 		echo "header #menulava > li.current-menu-item > a, header #menulava > li.current-menu-ancestor > a{ color: " . $anya_styleColor . ";}";
 		} ?>
 		...
 		.shortcode-team .team-box h5, .color_logo, #recentcomments a:hover, .recentcomments_listing a.the_title:hover, #accordion .acc-title h2.current, .acc-substitute .acc-title h2.current, .text_color, .woocommerce form .form-row .required, .woocommerce-page form .form-row .required, .cart_list_product_title a:hover, .mail_chimp_form_container input.button:hover,#footer_content .contact-form .submit:hover, #toppanel .contact-form .submit:hover, #footer_menu li a:hover, .wpb_content_element .wpb_tour_tabs_wrapper .wpb_tabs_nav a:hover, .wpb_content_element .wpb_accordion_header a:hover, h3.ui-state-active a, .wpb_tabs_nav .ui-state-active a, .wpb_toggle_title_active, #content h4.wpb_toggle_title_active, .widget_nav_menu .menu li a:hover, .woocommerce div.product .woocommerce-tabs ul.tabs li.active, .woocommerce-page div.product .woocommerce-tabs ul.tabs li.active, .recentPosts .post_type i, .post-listing .post_type i{
	 		<?php echo "color: " . $anya_styleColor . " !important;"; ?>
 		}
	 		...
	 		header #menulava > li > a:hover, .sf-menu li li a:hover, .footer_right_content .footer_menu li a:hover, .sf-menu > li li.sfHover > a, .sfHover > a{
		 		<?php 
		 			if (strpos($_SERVER['HTTP_USER_AGENT'], 'Trident/7.0; rv:11.0') !== false) {
					    echo "color: #".des_get_value(DESIGNARE_SHORTNAME.'_menu_selected_color')."; background: #".des_get_value(DESIGNARE_SHORTNAME.'_menu_selected_background_color')." !important;";
					} else echo "color: #".des_get_value(DESIGNARE_SHORTNAME.'_menu_selected_color')." !important; background: #".des_get_value(DESIGNARE_SHORTNAME.'_menu_selected_background_color')." !important;"; ?>
	 		}
	 		<?php 
	 	}
 		?>
 		...
 		#tabs ul.tabs li a.current{
	 		<?php echo "border-top: 1px solid " . $anya_styleColor . ";"; ?>
 		}
 		
 		.page_title .arrows-proj2 .next2:hover, .special_tabs .label.current .designare_icon_special_tabs, .filterby_btn{
	 		<?php echo "border: 1px solid " . $anya_styleColor . ";"; ?>
 		}
 		.arrow-right{
	 		<?php echo "border-left: 5px solid " . $anya_styleColor . ";"; ?>
 		}
 		...
 		#flickr li:hover{
 			<?php echo "border: 3px solid ".$anya_styleColor .";"; ?>
 		}
 		
 		<?php 
	 		if ($headerStyleType === "style1" || $headerStyleType === "style2"){
		 		echo "header #menulava > li:hover > a{ color:".$anya_styleColor." !important;}";
	 		}
 		?>
 		
 		.project_list_s3 .p_title a:hover, .testi-info .company, .the_title a:hover, .numerical-container .unit, .tooltiper span, .des-sc-button.button.custom:hover, .tweet_time{
	 		<?php echo "color: " . $anya_styleColor . " !important;"; ?>
 		}
 		 		
 		.pricing_tab.highlight{
 			<?php echo "border: 1px solid " . $anya_styleColor . " !important;"; ?>
 		}
		...
		#wp-calendar tbody td a, ins{ background-color: <?php echo $anya_styleColor; ?>;}

		.woocommerce .star-rating span, .woocommerce-page .star-rating span{
			color: <?php echo $anya_styleColor; ?>;
		}
		...
		#big_footer, #footer_content h4, .footer-widget h4, .footer-widget h4.page_title_testimonials, #footer_content .title h4, #footer_content .contact-form textarea, #footer_content .contact-form input{
			<?php
				...
				if (empty($bigfootertype)) $bigfootertype = des_get_value(DESIGNARE_SHORTNAME."_footerbg_type option:selected");
				if ($bigfootertype == "color") echo "background: #" . des_get_value(DESIGNARE_SHORTNAME."_footerbg_color") . " !important;";
				if ($bigfootertype == "pattern") echo "background: url('" . get_template_directory_uri() . "/images/anya_patterns/" . des_get_value(DESIGNARE_SHORTNAME."_footerbg_pattern") . "') 0 0 repeat !important;";
		...
		.mask .more .icon-search{background-color: <?php echo $anya_styleColor; ?>; border-radius:100%;-webkit-border-radius:100%;-moz-border-radius:100%;}
		...
		<?php
				...
				if ($bodyLayoutType != "boxed"){
					...
					if (des_get_value(DESIGNARE_SHORTNAME."_info_above_menu") == "on"){
						if ($headerStyleType === "style4" || $headerStyleType === "style1"){
							echo ".fullwidth-container{ margin-top: ". intval($value+139) ."px;}";
						} else {
							echo ".fullwidth-container{ margin-top: ". intval($value+55) ."px;}";
						}
					} else {
						if ($headerStyleType === "style4"){
							echo ".fullwidth-container{ margin-top: ". intval($value+95) ."px;}";
						} else {
			...
			if ($headerStyleType === "style3"){
				echo "
					...
					#menulava > li.current-menu-item, #menulava > li.current-menu-ancestor, #menulava > li.current-menu-ancestor > a{background:".$anya_styleColor." !important;}
			...
			if ($headerStyleType === "style4"){
				if (des_get_value(DESIGNARE_SHORTNAME."_menu_background_color")){
					echo ".fullwidth_container_menu, .fullwidth_container_menu #menulava li, .fullwidth_container_menu .sub-menu a, .headerstyle-style3 #menulava .sub-menu, .anya_minicart, #lang_sel ul ul li, #lang_sel_click ul ul li{ background:#".des_get_value(DESIGNARE_SHORTNAME."_menu_background_color")." !important;}";
				}
				...
				echo "
					...
					#menulava > li {margin-right:0px !important;}
					#menulava > li.current-menu-item, #menulava > li.current-menu-ancestor, #menulava > li.current-menu-ancestor > a{color: #fff !important;}
					
					#menulava > li:hover{border-top:none;}
					#menulava > li a:hover, .headerstyle-style4 #menulava > li.current-menu-ancestor > a, .sf-menu .current-menu-item > a{color: #".des_get_value(DESIGNARE_SHORTNAME.'_menu_selected_color')." !important;background: #".des_get_value(DESIGNARE_SHORTNAME.'_menu_selected_background_color')." !important;}
					.headerstyle-style4 .anya_dynamic_shopping_bag{background: #".des_get_value(DESIGNARE_SHORTNAME.'_menu_selected_background_color')." !important;}
				";
			}
			...
			if ($headerType === "without") {
				echo ".fullwidth-container{display:none !important;opacity:0;} #wrapper{margin-top:20px;}";
			}
			
			if (get_option("enable_custom_css") == "on"){
				$anya_customcss = get_option(DESIGNARE_SHORTNAME."_custom_css");
				if (gettype($anya_customcss) === "string" && $anya_customcss != "") {
					echo strip_tags($anya_customcss);
				}
			}
		?>
		...
		.mail_chimp_form_container input.button{
			background-color: <?php echo $anya_styleColor; ?> !important;
		...
		.mail_chimp_form_container input.button:hover, .style-top-bar .info_above_menu a.anya_minicart_cart_but:hover, .style-top-bar .info_above_menu a.anya_minicart_checkout_but:hover{
			color: <?php echo $anya_styleColor; ?> !important;
		}
		.mail_chimp_form_container input.button:hover, .style-top-bar .info_above_menu a.anya_minicart_cart_but:hover, .style-top-bar .info_above_menu a.anya_minicart_checkout_but:hover{
			background: <?php echo $anya_styleColor; ?> !important;
			color: #fff !important;
		}
...
function designare_anya_custom_head(){
...
function designare_anya_style() {
	wp_enqueue_style( 'designare-anya-style', get_bloginfo( 'stylesheet_url' ), array(), '1' );
	wp_enqueue_style( 'des-googlefont-opensans', 'http'. ( is_ssl() ? 's' : '' ) .'://fonts.googleapis.com/css?family=Open+Sans:400,300,600,700,800', array(), '1', 'screen' );
	wp_enqueue_style( 'des-googlefont-raleway', 'http'. ( is_ssl() ? 's' : '' ) .'://fonts.googleapis.com/css?family=Raleway:400,300,700,800', array(), '2', 'screen' );
}
...
function designare_scripts(){
	...
	if (!is_admin()){
	    ...
	    wp_enqueue_script( 'animated-contents', DESIGNARE_JS_PATH .'animated-contents.js', array(),'1.0',$in_footer = true);
  	    ...
  	    wp_enqueue_script( 'anya', DESIGNARE_JS_PATH .'anya.js', array(), '1',$in_footer = true);
			    ...
			    switch($template){
				    case 'blog-template.php': case 'blog-template-fullwidth.php': case 'blog-template-leftsidebar.php': case 'index.php': case 'post-archive.php': case 'post-single.php': case 'search.php':
						...
						wp_enqueue_script('blog', get_template_directory_uri().'/js/blog.js', array(), '1',$in_footer = true);
				   	    if (get_option('anya_blog_reading_type') === "scroll" || get_option('anya_blog_reading_type') == "scrollauto"){
					   	 	wp_enqueue_style('load-posts', DESIGNARE_CSS_PATH.'load-posts.css');   
				   	    }
					    break;
					default:
...
function designare_the_breadcrumb(){
    ...
    if (!is_front_page()) {         
        ...
        elseif ( is_search() ) {  
            echo __('Search Results for "', 'anya') . get_search_query() . '"';
        }       
        ...
        elseif ( is_author() ) {
            ...
            echo  __('Author&#39;s Article(s) &raquo; ', 'anya') . $user_info->display_name ;
        }       
        elseif ( is_404() ) {
...
function my_theme_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			...
			'source'   				=> get_template_directory_uri() . '/lib/plugins/really-simple-captcha.1.7.zip', // The plugin source
			'required' 				=> true, // If false, the plugin is only 'recommended' instead of required
			'version' 				=> '', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
			'force_activation' 		=> false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
			'force_deactivation' 	=> false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
		...
		array(
			'name'     				=> 'DK Pricr - Responsive Pricing Table', // The plugin name
			'slug'     				=> 'dk-pricr-responsive-pricing-table', // The plugin slug (typically the folder name)
			'source'   				=> get_template_directory_uri() . '/lib/plugins/dk-pricr-responsive-pricing-table.zip', // The plugin source
			'required' 				=> true, // If false, the plugin is only 'recommended' instead of required
			'version' 				=> '', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
			'force_activation' 		=> false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
			'force_deactivation' 	=> false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
...
function custom_excerpt_more( $more ) {
	return ' <a class="read-more-excerpt" href="' . get_permalink( get_the_ID() ) . '">' . __( '[…]', 'your-text-domain' ) . '</a>';
}
add_filter( 'excerpt_more', 'custom_excerpt_more' );


function get_post_meta_or_default ($post_id, $meta_key, $default) {
...
function get_post_meta_or_default_with_prefix ($post_id, $meta_key, $default, $prefix) {
    return $prefix . get_post_meta_or_default($post_id, $meta_key, $default);
}
