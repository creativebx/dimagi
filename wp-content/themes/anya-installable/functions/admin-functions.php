...
function des_title(){
	...
	if($use_wp_title == 'true'){
		...
		elseif ( is_search() ) { $title = get_bloginfo( 'name') . $sep . __( 'Search Results', 'anya' );  }
		elseif ( is_author() ) { $title = get_bloginfo( 'name') . $sep . __( 'Author Archives', 'anya' );  }
		...
		elseif ( is_category() ) { $title = get_bloginfo( 'name') . $sep . __( 'Category Archive', 'anya' ) . $sep . single_cat_title( '',false );  }
		elseif ( is_tax() ) { $title = get_bloginfo( 'name') . $sep . $taxonomy_top_level_item . __( ' Archive', 'anya' ) . $sep . $taxonomy_nice_name;  }
		elseif ( is_day() ) { $title = get_bloginfo( 'name') . $sep . __( 'Daily Archive', 'anya' ) . $sep . get_the_time( 'jS F, Y' );  }
		elseif ( is_month() ) { $title = get_bloginfo( 'name') . $sep . __( 'Monthly Archive', 'anya' ) . $sep . get_the_time( 'F' );  }
		elseif ( is_year() ) { $title = get_bloginfo( 'name') . $sep . __( 'Yearly Archive', 'anya' ) . $sep . get_the_time( 'Y' );  }
		elseif ( is_tag() ) {  $title = get_bloginfo( 'name') . $sep . __( 'Tag Archive', 'anya' ) . $sep . single_tag_title( '',false); }
		elseif ( function_exists( 'is_post_type_archive' ) && is_post_type_archive() ) { $title = get_bloginfo( 'name') . $sep . $archive_name . __( ' Archive', 'anya' );  }
	}
...
if ( ! function_exists( 'des_pagination' ) ) {
	...
	function des_pagination( $args = array(), $query = '' ) {
		...
		$defaults = array(
			...
			'current' => $current,
			'prev_next' => true,
			'prev_text' => __( '&laquo; Previous', 'anya' ), // Translate in WordPress. This is the default.
			'next_text' => __( 'Next &raquo;', 'anya' ), // Translate in WordPress. This is the default.
			'show_all' => false,
			'end_size' => 1,
			'mid_size' => 1,
			'add_fragment' => '',
...
function des_breadcrumbs( $args = array() ) {
	...
	$defaults = array(
		'separator' => '&gt;',
		'before' => '<span class="breadcrumb-title">' . __( 'You are here:', 'anya' ) . '</span>',
		'after' => false,
		'front_page' => true,
		'show_home' => __( 'Home', 'anya' ),
		'echo' => true, 
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_time() ) {
			...
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g:i a', 'anya' ) );

			elseif ( get_query_var( 'minute' ) )
				$trail['trail_end'] = sprintf( __( 'Minute %1$s', 'anya' ), get_the_time( __( 'i', 'anya' ) ) );

			elseif ( get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g a', 'anya' ) );
		}
		...
		elseif ( is_date() ) {
			...
			if ( is_day() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'anya' ) ) . '">' . get_the_time( __( 'Y', 'anya' ) ) . '</a>';
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'anya' ) ) . '">' . get_the_time( __( 'F', 'anya' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'j', 'anya' ) );
			}

			elseif ( get_query_var( 'w' ) ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'anya' ) ) . '">' . get_the_time( __( 'Y', 'anya' ) ) . '</a>';
				$trail['trail_end'] = sprintf( __( 'Week %1$s', 'anya' ), get_the_time( esc_attr__( 'W', 'anya' ) ) );
			}

			elseif ( is_month() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'anya' ) ) . '">' . get_the_time( __( 'Y', 'anya' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'F', 'anya' ) );
			}

			elseif ( is_year() ) {
				$trail['trail_end'] = get_the_time( __( 'Y', 'anya' ) );
			}
		}
	}
	...
	elseif ( is_search() )
		$trail['trail_end'] = sprintf( __( 'Search results for &quot;%1$s&quot;', 'anya' ), esc_attr( get_search_query() ) );

	/* If viewing a 404 error page. */
	elseif ( is_404() )
		$trail['trail_end'] = __( '404 Not Found','anya' );
...
function des_admin_bar_menu () {
	...
	// Theme Options
	$wp_admin_bar->add_menu( array( 'parent' => 'desthemes', 'id' => 'desthemes-theme-options', 'title' => __( 'Theme Options', 'anya' ), 'href' => admin_url( 'admin.php?page=desthemes' ) ) );

	// Sidebar Manager
	if ( get_option( 'framework_des_sbm_disable') != 'true' ) {
		$wp_admin_bar->add_menu( array( 'parent' => 'desthemes', 'id' => 'desthemes-sbm', 'title' => __( 'Sidebar Manager', 'anya' ), 'href' => admin_url( 'admin.php?page=desthemes_sbm' ) ) );
	}

	if ( ( $super_user == $current_user_id ) || empty( $super_user ) ) {

		// Framework Settings
		$wp_admin_bar->add_menu( array( 'parent' => 'desthemes', 'id' => 'desthemes-framework-settings', 'title' => __( 'Framework Settings', 'anya' ), 'href' => admin_url( 'admin.php?page=desthemes_framework_settings' ) ) );

		// Update Framework
		$wp_admin_bar->add_menu( array( 'parent' => 'desthemes', 'id' => 'desthemes-update-framework', 'title' => __( 'Update Framework', 'anya' ), 'href' => admin_url( 'admin.php?page=desthemes_framework_update' ) ) );
