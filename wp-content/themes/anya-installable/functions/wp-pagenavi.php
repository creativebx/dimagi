...
function wp_pagenavi($before = '', $after = '') {
	...
	if (!$wp_query->is_single) {
		if (!$wp_query->is_search){
			...
			$counter = $counter->post_count;
		}
		//$res = $wpdb->get_results($request);
		$posts_per_page = get_option('posts_per_page');
		$paged = $pag;
		$pagenavi_options = get_option('pagenavi_options');
		$max_page = ceil($counter / $posts_per_page);
		...
		if($max_page > 1 || intval($pagenavi_options['always_show']) == 1) {
			...
			$pages_text = __("Page ","anya") . $paged . __(" of ","anya") . $max_page;
			...
			switch(intval($pagenavi_options['style'])) {
				...
				case 2:
					...
					echo '<label class="gotopagedd" for="pagedd">'. __('Go to page','anya') .'</label>';
...
function pagenavi_init() {
	...
	$pagenavi_options['pages_text'] = __('Page _CURRENT_PAGE_ of _TOTAL_PAGES_','anya');
	$pagenavi_options['current_text'] = '%PAGE_NUMBER%';
	$pagenavi_options['page_text'] = '%PAGE_NUMBER%';
	$pagenavi_options['first_text'] = __('&laquo;','anya');
	$pagenavi_options['last_text'] = __('&raquo;','anya');
	$pagenavi_options['next_text'] = __('&raquo;','anya');
	$pagenavi_options['prev_text'] = __('&laquo;','anya');
	$pagenavi_options['dotright_text'] = __('...','anya');
	$pagenavi_options['dotleft_text'] = __('...','anya');
